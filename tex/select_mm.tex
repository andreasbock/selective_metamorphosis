...
$\nu(x)=0$  implies $z_t=0$, this the template does not move. We now show that
the shooting system \eqref{eq-m-selective} is invertible:
\begin{theorem}
...
\begin{minipage}{\textwidth}
  \centering
    \includegraphics[width=.46\textwidth]{\resultPath results_criss_cross_10/centroid_heat.pdf}
    \includegraphics[width=.46\textwidth]{density_criss_cross.pdf}\\[0.23cm]
    \includegraphics[width=.46\textwidth]{\resultPath results_criss_cross_10/functional_histogram.pdf}
    \includegraphics[width=.46\textwidth]{\resultPath results_criss_cross_10/autocorrelation.pdf}
    %\includegraphics[width=.23\textwidth]{\resultPath results_criss_cross_10/MAP_center_0.pdf}
    %\includegraphics[width=.23\textwidth]{\resultPath results_criss_cross_10/MAP_center_1.pdf}
    %\includegraphics[width=.23\textwidth]{\resultPath results_criss_cross_10/MAP_center_2.pdf}
    %\subcaption{.}
...
\cite{richardson2016metamorphosis} or developing a space-time method. Further,
as outlined in \ref{sec:bayesian} there are many aspects of the probabilistic
framework that need rigorous treatment. Beyond the references therein, see also
the work in \cite{dashti2013map}. Natural extensions of our probabilistic ansatz
also includes fully treating $\nu$ as a function and interpreting the resulting
inverse problem through the appropriate measure-theoretical lens. Adding a
time-dependency to $\nu$ is also to be explored. Determining truncated Fourier
modes of $\nu$ could lead to efficient numerical methods. More generally, we
hope to reconcile our attempts to model growth here with the elegant approach
described by Kaltenmark in \cite{kaltenmark2016geometrical}. See also
\cite{kaltenmark2016partial,kaltenmark2017estimation,kaltenmark2017existence}.\\
...
$(\mathbf q,\, u)\mapsto \frac{\dot{\mathbf q_t} - u(\mathbf q_t)}{\sqrt{\nu(\mathbf q_t)}} := \mathbf w$:
\begin{align*}
\Big(\sum_{i=1}^M |w_t^i|\Big)^{\frac 12} & = \ltwonorm{\mathbf w} =
\ltwonorm{\frac{\dot{\mathbf q}_t - u(\mathbf q_t)}{\sqrt{\nu(\mathbf q_t)}}}\\
& \lesssim \nuinf^{-1}\Big(\ltwonorm{\dot{\mathbf q}_t} + \vnorm{u(\mathbf
q_t)}\Big)
\end{align*}
which is clearly bounded since $\mathbf q\in H^1([0,1],\RdM)$ and by the
conservation of $u$. From this we generate a minimising sequence $(\mathbf q^n,
u^n, \mathbf w^n)_{n\geq 0}$ admissible to \eqref{pbl:reformulation}. Next we
show the constraint equation is continuous with respect to the weak topology
on $X\triangleq H^1([0,1],\RdM)\times L^2([0,1],V)\times L^2([0,1],\RdM)$
i.e. $\langle e(\mathbf q^n,\,\mathbf w^n,\,\mathbf u^n)-e(\mathbf q,\,\mathbf
w,\,\mathbf u),\,\phi\rangle_{X,\,X^*}\rightarrow 0$, $\forall \phi\in X^*$
where $e(q,\,w,\,u) \triangleq \dot{q} - u(q) - \sqrt{\nu{q_t}}w_t$. Here, the
pairing is understood in the weak sense. Then,
\begin{align*}
\langle\sqrt{\nu(q)}w - \sqrt{\nu(q^n)}w^n,\,\phi\rangle \lesssim \nuinf\langle
w - w^n,\,\phi\rangle \rightarrow 0
\end{align*}
\begin{align*}
\langle u(q)-u^n(q^n),\,\phi\rangle & = \langle u(q)- u^n(q),\,\phi\rangle + \langle u^n(q)-u^n(q^n),\,\phi\rangle\\
\end{align*}
the first term vanishes trivially, while we invoke \cite[Lemma 7]{trouve2005local}
to treat the second:
\begin{align*}
\langle u^n(q)-u^n(q^n),\,\phi\rangle \lesssim e^{C\int_0^1\vnorm{u^n}\diff{t}}\langle q-q^n,\,\phi\rangle \rightarrow 0
\end{align*}
...
classic results \cite{younes2010shapes} extract bounded subsequences converging
to weak limits where necessary to obtain a minimiser. Convexity of $S$ implies
